...
const QUIZ_DATA = {
  "Algebra / Pre-Calc": {
    "High School": [
      { id:"alg-hs-1", difficulty:"Easy",   hint:"Move the constant to the other side first, then divide.", question:"Solve for x: 2x + 5 = 17", options:["x = 4","x = 6","x = 11","x = 3"], answer:1, explanation:"Subtract 5 from both sides: 2x = 12, then divide by 2: x = 6." },
      { id:"alg-hs-2", difficulty:"Easy",   hint:"In y = mx + b, m is always the slope.", question:"Which of the following is the slope of the line y = -3x + 7?", options:["7","3","-3","-7"], answer:2, explanation:"In slope-intercept form y = mx + b, m is the slope. Here m = -3." },
      { id:"alg-hs-3", difficulty:"Medium", hint:"This is a difference of squares: a² - b² = (a+b)(a-b).", question:"Factor: x² - 9", options:["(x-3)²","(x+3)(x-3)","(x-9)(x+1)","(x+9)(x-1)"], answer:1, explanation:"Difference of squares: a² - b² = (a+b)(a-b), so x² - 9 = (x+3)(x-3)." },
      { id:"alg-hs-4", difficulty:"Medium", hint:"Vertex form is f(x) = (x-h)² + k where vertex is (h, k).", question:"What is the vertex of f(x) = (x - 2)² + 5?", options:["(-2, 5)","(2, -5)","(2, 5)","(-2, -5)"], answer:2, explanation:"In vertex form f(x) = (x-h)² + k, vertex is (h,k). Here h=2, k=5." },
      { id:"alg-hs-5", difficulty:"Medium", hint:"Multiply coefficients together and add exponents of like variables.", question:"Simplify: (3x²y)(4xy³)", options:["12x³y⁴","7x³y⁴","12x²y³","12x³y³"], answer:0, explanation:"Multiply coefficients: 3×4=12. Add exponents: x²⁺¹=x³, y¹⁺³=y⁴." },
    ],
    "AP / Pre-College": [
...
.sb-prog-step.done-correct { background: #34d399; }
.sb-prog-step.done-wrong   { background: #f87171; }
.sb-prog-step.current      { background: #a78bfa; }
...
.sb-diff-Easy   { background: rgba(52,211,153,0.12);  color: #34d399; border: 1px solid rgba(52,211,153,0.3);  }
.sb-diff-Medium { background: rgba(251,191,36,0.12);  color: #fbbf24; border: 1px solid rgba(251,191,36,0.3);  }
.sb-diff-Hard   { background: rgba(248,113,113,0.12); color: #f87171; border: 1px solid rgba(248,113,113,0.3); }

/* Difficulty filter buttons on home card */
.sb-diff-filter {
  display: flex;
  gap: 6px;
  flex-wrap: wrap;
...
.sb-diff-filter-btn:hover { opacity: 0.85; transform: scale(1.04); }
...
.sb-hint-btn:hover { background: rgba(251,191,36,0.15); border-color: rgba(251,191,36,0.45); }
.sb-hint-btn:disabled { opacity: 0.4; cursor: not-allowed; }
...
export default function StudyBuddyQuiz() {
  ...
  const startQuiz = (subject, level, difficulty = "All") => {
    let qs = QUIZ_DATA[subject]?.[level];
    if (!qs?.length) return;
  ...
  const handleAnswer = (idx) => {
    ...
    setAnswers(a=>[...a,{chosen:idx, correct:ok, hintUsed}]);
  };
              ...
              <div key={subj} className={`sb-card sb-a${Math.min(si,4)}`}>
                ...
                <div className="sb-level-list">
                  {LEVELS.map(level=>{
                    ...
                    const difficulties = ["All", "Easy", "Medium", "Hard"].filter(d =>
                      d === "All" || qs.some(q => q.difficulty === d)
                    );
                    const diffBtnColors = {
                      All:    { bg:"rgba(139,92,246,0.1)",  color:"#a78bfa", border:"rgba(139,92,246,0.3)"  },
                      Easy:   { bg:"rgba(52,211,153,0.1)",  color:"#34d399", border:"rgba(52,211,153,0.3)"  },
                      Medium: { bg:"rgba(251,191,36,0.1)",  color:"#fbbf24", border:"rgba(251,191,36,0.3)"  },
                      Hard:   { bg:"rgba(248,113,113,0.1)", color:"#f87171", border:"rgba(248,113,113,0.3)" },
                    };
                    return (
                      <div key={level} style={{marginBottom:4}}>
                        <div className="sb-lvl-btn" style={{background:lm.bg, color:lm.color, borderColor:lm.border, cursor:"default", marginBottom:5}}>
                          <span className="sb-lvl-dot" style={{background:lm.color}}/>
                          {level}
                          <span className="sb-lvl-count">{qs.length} Q</span>
                        </div>
                        <div className="sb-diff-filter" style={{paddingLeft:8}}>
                          {difficulties.map(d => {
                            const dc = diffBtnColors[d];
                            return (
                              <button key={d} className="sb-diff-filter-btn"
                                style={{background:dc.bg, color:dc.color, borderColor:dc.border}}
                                onClick={()=>startQuiz(subj, level, d)}
                              >
                                {d === "All" ? `All (${qs.length})` : `${d} (${qs.filter(q=>q.difficulty===d).length})`}
                              </button>
                            );
                          })}
                        </div>
  ...
  if (screen === "quiz") {
    ...
    const diffColor = diffColors[q.difficulty] || "#a78bfa";
        ...
        <div className="sb-inner">
          ...
          <div className="sb-quiz-body">
            ...
            <div className="sb-prog-row">
              ...
              <span>{score} correct · {currentQ - score} wrong</span>
            </div>
            <div className="sb-prog-track">
              <div className="sb-prog-fill" style={{width:`${progress}%`}}/>
            </div>
            <div className="sb-prog-steps">
              {questions.map((_,i) => {
                let cls = "sb-prog-step";
                if (i < currentQ) cls += answers[i]?.correct ? " done-correct" : " done-wrong";
            ...
            <div className="sb-q-card sb-fade">
              <div className="sb-q-label" style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                <span>Question {currentQ+1}</span>
                {q.difficulty && (
                  <span className={`sb-diff-badge sb-diff-${q.difficulty}`}>
            ...
            {/* Hint button — only show if question has a hint and not yet answered */}
            {q.hint && selectedAnswer === null && (
              <button
              ...
              >
                ...
                {!showHint && <span style={{fontSize:10,opacity:0.6,marginLeft:4}}>(-½ pt)</span>}
              </button>
            )}

            {/* Hint box */}
            {showHint && q.hint && (
              <div className="sb-hint-box sb-fade">
                <span className="sb-hint-label">💡 Hint</span>
                {q.hint}
              </div>
            )}
              ...
              <div className="sb-expl sb-fade">
                <div className="sb-expl-ttl" style={{color: selectedAnswer===q.answer?"#34d399":"#f87171"}}>
                  {selectedAnswer===q.answer
                    ? hintUsed ? "✓  Correct (with hint)" : "✓  Correct"
                    : "✗  Incorrect"}
                </div>
  ...
  if (screen === "retry") return (
    <RetryQuiz
      questions={questions}
      wrongIndexes={answers.map((a,i) => !a.correct ? i : -1).filter(i => i !== -1)}
      wrongAnswers={answers.map((a,i) => !a.correct ? a.chosen : -1).filter(i => i !== -1)}
          ...
          <div className="sb-results-body">
            <div className="sb-results-card sb-fade">
              ...
              <div className="sb-results-actions">
                {pct < 100 && (
                  <button className="sb-btn-primary" onClick={()=>setScreen("retry")}>
                    🔁 Retry Wrong Answers ({questions.length - score})
                  </button>
                )}
                {pct === 100 && (
